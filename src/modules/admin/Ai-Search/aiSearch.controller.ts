...
// aiSearch.controller.ts
import { Request, Response } from "express";
import { calculateMatchScore, updateApplicantMatchScoreAndInsights } from "./aiSearch.service";
...
export async function evaluateApplicant(req: Request, res: Response) {
  try {
    ...
    const { matchScore, insights } = await calculateMatchScore(jobDescription, resumeBase64);

    // Update the applicant's match score and insights in the database
    await updateApplicantMatchScoreAndInsights(applicantId, matchScore, insights);
